...
class Transaction
{
     ...
     * @return Argument
     */
    public function gasArgument(): Argument
    {
        return new Argument(0, 'GasCoin', 'object', 0, [], true);
    }
    ...
    {
        return json_encode($this->getV1Data());
    }
    ...
    public static function from(string|array|self $transaction): self
    {
        ...
        if ($transaction instanceof self) {
            $newTransaction->data = TransactionData::fromArray($transaction->getData()->toArray());
     ...
     * @return V1
     */
    public function getV1Data(): V1
    {
        return V1::serializeV1TransactionData($this->data->snapshot());
    }
     ...
     *
     * @param string|Serialized $value The value to create a pure argument from
     * @return Argument The created argument
     */
    public function pure(string|Serialized $value): Argument
    {
     ...
     *
     * @param string|Argument $coin The coin to split
     * @param array<int|string|Argument> $amounts The amounts to split into
     * @return Argument The result of the split operation
     */
    public function splitCoins(string|Argument $coin, array $amounts): Argument
    {
        $coinArg = is_string($coin) ? $this->object($coin) : $coin;
        $amountArgs = array_map(
            fn($amount) => is_numeric($amount) ? $this->pure((string)$amount) : $amount,
            $amounts
        );
        return $this->add(Commands::splitCoins($coinArg, $amountArgs));
    }

    /**
     * Merge multiple coins into a destination coin
     *
     * @param string|Argument $destination The destination coin
     * @param array<string|Argument> $sources The source coins to merge
     * @return Argument The result of the merge operation
     */
    public function mergeCoins(string|Argument $destination, array $sources): Argument
    {
        $destArg = is_string($destination) ? $this->object($destination) : $destination;
        $sourceArgs = array_map(
            fn($source) => is_string($source) ? $this->object($source) : $source,
            $sources
        );
        return $this->add(Commands::mergeCoins($destArg, $sourceArgs));
    }

    /**
     * Publish a Move module
     *
     * @param array<mixed> $params The publish parameters
     * @return Argument The result of the publish operation
     */
    public function publish(array $params): Argument
    {
        return $this->add(Commands::publish($params));
    }

    /**
     * Upgrade a Move module
     *
     * @param array<mixed> $params The upgrade parameters
     * @return Argument The result of the upgrade operation
     */
    public function upgrade(array $params): Argument
    {
        $params['ticket'] = is_string($params['ticket']) ? $this->object($params['ticket']) : $params['ticket'];
        return $this->add(Commands::upgrade($params));
    }

    /**
     * Make a Move call
     *
     * @param array<mixed> $params The move call parameters
     * @return Argument The result of the move call
     */
    public function moveCall(array $params): Argument
    {
        if (isset($params['arguments'])) {
            $params['arguments'] = array_map(
                fn($arg) => $this->normalizeTransactionArgument($arg),
                $params['arguments']
            );
        }
        return $this->add(Commands::moveCall($params));
    }

    /**
     * Transfer objects to an address
     *
     * @param array<string|Argument> $objects The objects to transfer
     * @param string|Argument $address The destination address
     * @return Argument The result of the transfer operation
     */
    public function transferObjects(array $objects, string|Argument $address): Argument
    {
        $objectArgs = array_map(
            fn($obj) => is_string($obj) ? $this->object($obj) : $obj,
            $objects
        );
        $addressArg = is_string($address) ? $this->pure($address) : $address;
        return $this->add(Commands::transferObjects($objectArgs, $addressArg));
    }

    /**
     * Make a Move vector
     *
     * @param array<mixed> $params The make move vector parameters
     * @return Argument The result of the make move vector operation
     */
    public function makeMoveVec(array $params): Argument
    {
        $params['elements'] = array_map(
            fn($element) => is_string($element) ? $this->object($element) : $element,
            $params['elements']
        );
        return $this->add(Commands::makeMoveVec($params));
    }

    /**
     * Normalize a transaction argument
     *
     * @param mixed $arg The argument to normalize
    ...
    private function normalizeTransactionArgument(mixed $arg): Argument
    {
        ...
        throw new \InvalidArgumentException('Invalid transaction argument');
    }
}
